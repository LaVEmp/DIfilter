...
Orb_threshold = 0.2  # 0~1 float
Hamming_threshold = 150  # 0~1024 int, 第二道筛选, 卡得死一些
# Cosine_threshold = 200  # 0~1024 int

# The thresholds used for judgment
Distance_threshold = 70  # 0~100 int
...
def _get_data_beta():  # for test only, init function
    PreImageUrl = "http://assettest.hzncc.cn/HncWeb/group1/M00/19/60/CgEUdV8Lz4KAPVrWAAczgiK6WOY640.jpg"
    CurImageUrl = "http://assettest.hzncc.cn/HncWeb/group1/M00/19/59/CgEUdl8Lz4GAH_jEAAZwlGEoyq4356.jpg"
    CurImageClips = [{"id": 528516, "temperature": 0.0, "alarmVlPicLeftTopX": 1038.0, "alarmVlPicLeftTopY": 290.0,
                      "alarmVlPicRightBottomX": 1154.0, "alarmVlPicRightBottomY": 406.0},
                     {"id": 528517, "temperature": 41.8, "alarmVlPicLeftTopX": 435.0, "alarmVlPicLeftTopY": 279.0,
                      "alarmVlPicRightBottomX": 546.0, "alarmVlPicRightBottomY": 388.0}]
    PreImageClips = [{"id": 528518, "temperature": 41.7, "alarmVlPicLeftTopX": 862.0, "alarmVlPicLeftTopY": 350.0,
                      "alarmVlPicRightBottomX": 982.0, "alarmVlPicRightBottomY": 470.0}]
...
def get_image_clips(imageUrl, imageClips):
    ...
    for image_coord in imageClips:
        image_coord = list(map(int, image_coord))
...
def compare_To(img1, img2):
    ...
    for clip1 in img1['img_clips']:
        ...
        [x1, y1, x2, y2] = [max(0, x1 - width_expand), max(0, y1 - height_expand), min(x2 + width_expand, raw_width),
                            min(y2 + height_expand * 2, raw_height)]
        ...
        index = max(max_orb_similarity_index, min_hamming_dis_index)
        index_coord = img2['img_clips'][index]['coord'] if index != -1 else -1

        index_final.append(index)
        coord_final.append(index_coord)  # [[x1, y1, x2, y2],-1,[],-1,...]
    ...
    return min(orb_similarity) > Orb_threshold or max(hamming_distance) < Hamming_threshold, index_final, coord_final
...
def is_Duplicated(PreImageUrl, CurImageUrl, PreImageClips, CurImageClips):
    ...
    if img1_to_img2 and img2_to_img1:
        # return True and _bool, unique_img1_coord, unique_img2_coord
        return True and _bool, unique_img2_coord
    else:
        return False, unique_img2_coord
